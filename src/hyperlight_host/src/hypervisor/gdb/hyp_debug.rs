...
pub mod mshv {
    ...
    use mshv_bindings::{DebugRegisters, StandardRegisters};
    ...
    impl GuestVcpuDebug for MshvDebug {
        ...
        fn get_stop_reason(&mut self, vcpu_fd: &VcpuFd, entrypoint: u64) -> Result<VcpuStopReason> {
            let regs = vcpu_fd
                .get_debug_regs()
                .map_err(|e| new_error!("Cannot retrieve debug registers from vCPU: {}", e))?;

            let debug_status_reg = regs.dr6;

            if 0x00000000_00008000 & debug_status_reg != 0 && self.single_step {
                return Ok(VcpuStopReason::DoneStep);
            }
            ...
            if 0x00000000_0000000F & debug_status_reg != 0 && self.hw_breakpoints.contains(&gpa) {
                // In case the hw breakpoint is the entry point, remove it to
                // avoid hanging here as gdb does not remove breakpoints it
                // has not set.
                // Gdb expects the target to be stopped when connected.
