...
use crate::hypervisor::handlers::{
    DbgGetCodeAddrHandlerFunction, DbgMemAccessHandler, DbgMemAccessHandlerWrapper, DbgReadMemAccessHandlerFunction, DbgWriteMemAccessHandlerFunction, MemAccessHandler, MemAccessHandlerFunction, MemAccessHandlerWrapper
};
...
#[instrument(err(Debug), skip_all, parent = Span::current(), level= "Trace")]
pub(super) fn handle_dbg_read_mem_access_impl(wrapper: &mut MemMgrWrapper<HostSharedMemory>, addr: usize, data: &mut [u8]) -> Result<()> {
    wrapper
        .unwrap_mgr_mut()
        .get_shared_mem_mut()
...
#[instrument(err(Debug), skip_all, parent = Span::current(), level= "Trace")]
pub(super) fn handle_dbg_write_mem_access_impl(wrapper: &mut MemMgrWrapper<HostSharedMemory>, addr: usize, data: &[u8]) -> Result<()> {
    wrapper
        .unwrap_mgr_mut()
        .get_shared_mem_mut()
...
#[instrument(err(Debug), skip_all, parent = Span::current(), level= "Trace")]
pub(super) fn handle_dbg_get_code_addr_impl(wrapper: &mut MemMgrWrapper<HostSharedMemory>) -> Result<usize> {
    Ok(wrapper
        .unwrap_mgr()
        .layout
...
pub(crate) fn dbg_mem_access_handler_wrapper(
    ...
    let read_access_func: DbgReadMemAccessHandlerFunction =
        Box::new(move |addr: usize, data: &mut [u8]| handle_dbg_read_mem_access_impl(&mut
                wrapper,
                addr,
                data,
                ));
    let write_access_func: DbgWriteMemAccessHandlerFunction =
        Box::new(move |addr: usize, data: &[u8]| handle_dbg_write_mem_access_impl(&mut wrapper2,
                addr,
                data,
                ));
    ...
    let dbg_mem_access_hdl = DbgMemAccessHandler::from((read_access_func, write_access_func, get_code_addr_func));
