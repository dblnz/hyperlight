...
impl GuestMemProfileProcessor {
    fn new(unwind_module: Arc<dyn crate::mem::exe::UnwindInfo>) -> Self {
...
impl TraceInfo {
    ...
    pub(crate) fn new(
        ...
        Ok(Self {
            ...
            #[cfg(feature = "mem_profile")]
            mem_profile: GuestMemProfileProcessor::new(unwind_module),
        })
    }
    ...
    pub(crate) fn process_trace_batch(
        ...
        for s in spans.iter() {
            ...
            let end_ts = s
                .end_tsc
                .map(|tsc| self.calculate_guest_time_relative_to_host(tsc));
