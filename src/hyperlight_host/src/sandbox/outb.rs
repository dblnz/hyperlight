...
fn handle_outb_impl(
    ...
    match port.try_into()? {
        ...
        OutBAction::TraceRecord => {
            ...
            let _trace_info = _hv.trace_info_as_mut();
            // Store the start guest cycles if not already set
            // This is the `entrypoint` of the guest execution
            if _trace_info.guest_start_tsc.is_none() && !traces.is_empty() {
                _trace_info.guest_start_tsc = Some(traces[0].cycles);
            }

            for record in traces {
                record_guest_trace_frame(_trace_info, 4u64, record.cycles, |f| {
